...
{
    private List<TK> _pendingKeys = new List<TK>();
    private List<TV> _pendingValues = new List<TV>();
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        // Render pending reference-type rows
        for (int i = 0; i < _pendingKeys.Count; i++)
        {
            position.y += 17f;
            ...
            {
                // Reference type (ScriptableObject, GameObject, etc.)
                newKey = (TK)(object)EditorGUI.ObjectField(
                    keyRect,
                    _pendingKeys[i] as UnityObject,
                    typeof(TK),
                    false
                );
            }
            ...
            {
                // Value type or other non-UnityObject
                newKey = DoField(keyRect, typeof(TK), _pendingKeys[i]);
            }

            if (EditorGUI.EndChangeCheck() && !EqualityComparer<TK>.Default.Equals(newKey, default))
            {
                // Add to dictionary once user assigns a valid key
                try
                {
                    _Dictionary.Add(newKey, _pendingValues[i]);
                }
                catch (Exception e)
                {
                    Debug.Log(e.Message);
            ...
            if (typeof(UnityObject).IsAssignableFrom(typeof(TV)))
            {
                _pendingValues[i] = (TV)(object)EditorGUI.ObjectField(
                    valueRect,
                    _pendingValues[i] as UnityObject,
                    typeof(TV),
                    false
                );
            }
            ...
            {
                _pendingValues[i] = DoField(valueRect, typeof(TV), _pendingValues[i]);
            }
        }
    ...
    {
        // Strings and value types: safe to add directly
        if (typeof(TK) == typeof(string) || typeof(TK).IsValueType)
        {
            var key = typeof(TK) == typeof(string) ? (TK)(object)"" : default(TK);
            _Dictionary.Add(key, default(TV));
            return;
        }

        // Reference-type keys: create a temporary row
        _pendingKeys.Add(default);
        _pendingValues.Add(default);
    }
